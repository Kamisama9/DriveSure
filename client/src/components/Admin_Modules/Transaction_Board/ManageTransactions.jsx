		...
		if (!hasActiveFilters) {
			...
			if (statusFilter !== "all") {
				filtered = filtered.filter(
					(transaction) => transaction.booking_status === statusFilter
				);
			}
		...
		<div className="p-4 flex flex-col min-h-screen">
			...
			<div className="mb-4">
				...
				<p className="text-gray-600">
					Manage and view all booking transactions
				</p>
			</div>
				...
				<div className="mt-4 pt-4 border-t border-gray-200">
					<p className="text-sm text-gray-600">
						{isFiltered
							...
							: `Showing latest ${Math.min(10, transactions.length)} of ${
									transactions.length
							  } transactions`}
					</p>
				</div>
				...
				>
					<table className="w-full h-full">
						<thead className="bg-gray-900 text-white sticky top-0 z-10">
							<tr>
								<th className="px-4 py-4 text-left text-sm font-semibold min-w-[120px]">
									Booking ID
								</th>
								<th className="px-4 py-4 text-left text-sm font-semibold min-w-[150px]">
									Rider
								</th>
								<th className="px-4 py-4 text-left text-sm font-semibold min-w-[150px]">
									Driver
								</th>
								<th className="px-4 py-4 text-left text-sm font-semibold min-w-[200px]">
									Route
								</th>
						...
						<tbody>
							{currentTransactions.map((transaction, index) => (
								...
								>
									...
									<td className="px-4 py-4 text-sm align-middle h-20">
										<div
											className="text-black font-medium truncate max-w-40"
											title={transaction.pickup_address}
										>
											From: {transaction.pickup_address}
										</div>
										<div
											className="text-gray-500 text-xs truncate max-w-40 mt-1"
											title={transaction.drop_address}
										>
